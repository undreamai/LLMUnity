...
namespace LLMUnity
{
    ...
    public class SentenceSplitter : MonoBehaviour
    {
        ...
        [DataMember]
        SortedDictionary<int, Phrase> phrases = new SortedDictionary<int, Phrase>();
        [DataMember]
        SortedDictionary<int, Sentence> sentences = new SortedDictionary<int, Sentence>();
        [DataMember]
        int nextPhraseId = 0;
        [DataMember]
        ...
        public async void Add(string text)
        {
            ...
            foreach ((int startIndex, int endIndex) in subindices)
            {
                ...
                await search.Add(sentenceId, sentenceText);
            }
        }
...
*/
        ...
        {
            return Search((await search.Embeddings(queryString)).ToArray(), k, out float[] distances, returnSentences);
        }
        ...
        public string[] Search(float[] encoding, int k, out float[] distances, bool returnSentences = false)
        {
            if (returnSentences)
            {
                int[] keys = search.Search(encoding, k, out distances);
            ...
            else
            {
                ...
                do
                {
                    int[] keys = search.Search(encoding, currK, out float[] iterDistances);
