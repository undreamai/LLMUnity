...
if [ "$(basename "$1")" == "README.md" ]; then
    ...
    EXISTING_CONTENT=`cat "$1" | sed -e 's:<p:<h1:g' | sed -e 's:c#::g' | sed -e 's:</p:</h1:g' | sed -e 's:\.github/:images/:g' | sed -e '/<sub>/,/<\/sub>/d' | sed -e 's:(Samples~:(https\://github.com/undreamai/LLMUnity/tree/main/Samples~:g'`
